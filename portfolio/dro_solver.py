...
class DROSolver:
    ...
    def compute_coverage(self, centers, radius, num_grid_points=50):
        ...
        corners = np.array(
            list(
                itertools.product(
                    *[[self.bounds[i, 0], self.bounds[i, 1]] for i in range(self.d)]
                )
            )
        )
        ...
        regular_grid = (
            np.array(
                np.meshgrid(
                    *[
                        np.linspace(
                            self.bounds[i, 0], self.bounds[i, 1], num_grid_points
                        )
    ...
    def find_minimum_radius(self, tolerance=1e-4, max_iterations=100):
        ...
        r_lower = (volume_orthant / (balls_per_orthant * volume_unit_ball)) ** (
            1 / self.d
        )
        ...
        while (r_upper - r_lower) > tolerance:
            ...
            for _ in range(max_iterations):
                ...
                initial_centers = np.random.uniform(
                    0, self.bounds[:, 1], size=(balls_per_orthant, self.d)
                )
    ...
    def solve_max_problem(
        self, x_current, weights_current, dro_params, max_iter_fw=1, y_current=None
    ):
        """
        ...
        def linear_oracle(grad):
            ...
            try:
                problem.solve(solver=cp.CLARABEL)  # or another suitable solver

                if problem.status != cp.OPTIMAL:
                    raise ValueError(
                        f"Linear oracle failed to solve optimally. Status: {problem.status}"
                    )
    ...
    def solve_min_problem_gd(
        ...
        perturbed_centers = np.array(
            [dro_params.ball_centres[k] + y_prev[k] for k in range(self.num_balls)]
        )
            ...
            for i in range(dro_params.x_dim):
                for k in range(self.num_balls):
                    ...
                    grad[i] += weights_current[k] * (
                        ...
                        + 2
                        * dro_params.c[i]
                        * perturbed_centers[k, i]
                        * (perturbed_centers[k, i] * x[i] - 1)
                    )
            return grad
    ...
    def visualize_samples_and_covering(self, samples, online_samples=None):
        ...
        plt.savefig("covering_visualization.pdf", bbox_inches="tight")
    ...
    def sample_from_mixture(self, size=1):  # now they are equally weighted
        ...
        for _ in range(size):
            ...
            if r < 0.25:  # Upper right quadrant
                mu_quad = np.mean(
                    [self.bounds[:, 1], (self.bounds[:, 0] + self.bounds[:, 1]) / 2],
                    axis=0,
                )
                ...
                mu_quad = np.mean(
                    [self.bounds[:, 0], (self.bounds[:, 0] + self.bounds[:, 1]) / 2],
                    axis=0,
                )
                sigma_quad = np.diag((self.bounds[:, 1] - self.bounds[:, 0]) / 4) ** 2
                sample = np.random.multivariate_normal(mu_quad, sigma_quad, size=1)
                ...
                mu_quad = np.array(
                    [
                        (self.bounds[0, 0] + self.bounds[0, 1]) / 4,
                ...
                mu_quad = np.array(
                    [
                        3 * (self.bounds[0, 0] + self.bounds[0, 1]) / 4,
    ...
    def solve_min_problem_exact(self, y_current, weights_current, dro_params):
        # Compute perturbed centers
        perturbed_centers = np.array(
            [dro_params.ball_centres[k] + y_current[k] for k in range(self.num_balls)]
        )
        ...
        for i in range(dro_params.x_dim):
            ...
            for k in range(self.num_balls):
                ...
                service_cost = (
                    dro_params.c[i]
                    * cp.power(x[i], 2)
                    * cp.power(perturbed_centers[k, i], 2)
                )
                linear_term = -2 * dro_params.c[i] * perturbed_centers[k, i] * x[i]
                constant_term = dro_params.c[i] - dro_params.d[i] * cp.power(
                    perturbed_centers[k, i], 2
                )

                resource_terms.append(
                    weights_current[k]
                    * (allocation_cost + service_cost + linear_term + constant_term)
                )
        ...
        try:
            ...
            if problem.status != cp.OPTIMAL:
                raise ValueError(
                    f"Min problem failed to solve optimally. Status: {problem.status}"
                )
    ...
    def compute_saa_solution(self, num_samples=1000, seed=1111):
        ...
        try:
            ...
            if problem.status != cp.OPTIMAL:
                raise ValueError(
                    f"SAA problem failed to solve optimally. Status: {problem.status}"
                )
    ...
    def compute_cumulative_regret(
        self, history, dro_params, online_samples, num_eval_samples=1000, seed=1111
    ):
        """
    ...
    def plot_regret_analysis(
        ...
        plt.semilogy(t_range, cumulative_regret, "k-", linewidth=2, label=r"Regret")

        theoretical_cumulative = 50 * np.sqrt(t_range)
        # theoretical_cumulative = 100 + np.sumsum(1 / np.sqrt(t_range))
        plt.plot(
            t_range, theoretical_cumulative, "k--", label=r"Theoretical $O(\sqrt{t})$"
        )
        ...
        plt.savefig("regret_analysis.pdf")
    ...
    def plot_computation_times(self, history):
        ...
        data = [history["computation_times"]["total_iteration"]]

        # Create boxplot
        bp = plt.boxplot(data, tick_labels=[r"Total Iteration"])
...
if __name__ == "__main__":
    ...
    initial_epsilon = (
        radius_init + solver.radius
    )  # Use solver.radius instead of ball_radius
    ...
    for t in range(T):
        ...
        if use_exact_solver:
            ...
            y_next = solver.solve_max_problem(
                x_current, weights, dro_params, max_iter_fw=1, y_current=y_current
            )
            max_time = time.time() - max_start_time
            ...
            x_next, min_obj = solver.solve_min_problem_gd(
                y_next, weights, dro_params, max_iter=1, x_current=x_current
            )
            min_time = time.time() - min_start_time
    ...
    cumulative_regret, optimal_hindsight, instantaneous_regret = (
        solver.compute_cumulative_regret(
            history, dro_params, np.array(online_samples), seed=simulation_seed
        )
    )

    # Plot regret analysis
    solver.plot_regret_analysis(
        cumulative_regret, optimal_hindsight, instantaneous_regret, history
    )
