...
from utils import calc_cluster_val as calc_cluster_val
...
def port_experiments(r_input, K, T, N_init, synthetic_returns, r_start):
    """Online mean-robust DRO-CVaR best-subset vs. batch (kmeans) MRO best-subset.

    Portfolio-MIP sibling of ``regression/reg_orig.py``.  The DRO best-subset
    MI-SOCP is replaced by the W1-DRO CVaR portfolio with a cardinality
    constraint (``createproblem_portMIP``, solved with MOSEK).  Data points are
    the asset-return vectors of dimension ``m`` clustered online / by kmeans;
    the worst-case CVaR of a fixed ``(x, tau)`` is the dual program
    ``worst_case``.

    A third method, ``cluster_SAA``, reuses the batch-MRO kmeans clusters but
    solves the *non-robust* (eps=0) cardinality-constrained scenario problem on
    those weighted centroids, isolating clustering from distributional robustness.
    """
    try:
        ...
        # leaves them well defined for the worst-case eval.
        x_current = np.ones(m) / m
        tau_current = 0.0
        MRO_x_current = np.ones(m) / m
        MRO_tau_current = 0.0
        cluster_SAA_x_current = np.ones(m) / m
        cluster_SAA_tau_current = 0.0
        ...
        for t in range(T):
            ...
            if t % interval == 0 or ((t - 1) % interval == 0) or (t in t_list):
                if t <= 8001 or (t in t_list):
                    ...
                    online_problem, online_x, online_s, online_tau, online_lmbda, data_train, eps_train, w_train = createproblem_portMIP(cur_K, m, card)
            ...
            # ---- solve batch MRO best-subset with fresh kmeans clusters ----
            if t % interval == 0 or ((t - 1) % interval == 0) or (t in t_list):
                if t <= 8001 or (t in t_list):
                    if t <= fixed_time:
                    ...
                    MRO_problem, MRO_x, MRO_s, MRO_tau, MRO_lmbda, MRO_data_train, MRO_eps_train, MRO_w_train = createproblem_portMIP(cur_K_mro, m, card)
                    ...
                    square_val_mro, sig_val_mro = calc_cluster_val(K, new_k_dict, num_dat, MRO_x_current, running_samples)[1:]

                    # cluster_SAA: non-robust scenario problem on the same weighted
                    # kmeans centroids (eps = 0), so it shares the clustering cost.
                    cs_prob, cs_x, cs_tau = create_scenario(new_k_dict['d'], m, num_dat, card, weights=new_k_dict['w'])
                    cs_prob.solve(solver=cp.MOSEK, ignore_dpp=True, verbose=False,
                                  mosek_params=MOSEK_PARAMS)
                    cluster_SAA_x_current = cs_x.value
                    cluster_SAA_tau_current = cs_tau.value
            ...
            # ---- worst-case CVaR (wrt full, non-clustered data) ----
            if t % interval == 0 or ((t - 1) % interval == 0) or (t in t_list):
                if t <= 8001 or (t in t_list):
                    ...
                    new_problem.solve(solver=cp.MOSEK, ignore_dpp=True, verbose=False,
                                      mosek_params=MOSEK_PARAMS)
                    ...
                    square_val, sig_val = calc_cluster_val(K, k_dict, num_dat, x_current, running_samples)[1:]

                    history['MRO_worst_values'].append(new_worst_MRO)
                    history['MRO_worst_times'].append(MRO_worst_time)

            # ---- worst-case CVaR wrt previous-stage solutions (for regret) ----
            if t % interval == 0 or ((t - 1) % interval == 0) or (t in t_list):
                if t <= 8001 or (t in t_list):
                    x_d.value = x_prev
            ...
            if t % interval == 0 or ((t - 1) % interval == 0) or (t in t_list):
                if t <= 8001 or (t in t_list):
                    ...
                    history['cluster_SAA_tau'].append(cluster_SAA_tau_current)
            ...
            if t % interval == 0 or ((t - 1) % interval == 0) or (t in t_list):
                if t <= 8001 or (t in t_list):
                    ...
                    df = pd.DataFrame({
                        ...
                        'cluster_SAA_tau': np.array(history['cluster_SAA_tau']),
