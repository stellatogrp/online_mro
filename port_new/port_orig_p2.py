...
def port_experiments(r_input,K,T,N_init,synthetic_returns,r_start):
    try:
        ...
        online_problem, online_x, online_s, online_tau, online_lmbda, data_train, eps_train, w_train = createproblem_portLP_p2(np.minimum(num_dat,K), m)
        MRO_problem, MRO_x, MRO_s, MRO_tau, MRO_lmbda, MRO_data_train, MRO_eps_train, MRO_w_train = createproblem_portLP_p2(np.minimum(num_dat,K), m)
        ...
        for t in range(T):
            ...
            if t % interval == 0 or ((t-1) % interval == 0) or (t in t_list):
                if t <= 8001 or (t in t_list):
                    # Rebuild the W2 problem on every solve.  The quad_over_lin
                    # perspective (||a*x||_2^2 / (4*lam)) makes Clarabel's
                    # in-place update path numerically fragile: after many
                    # successive parameter updates the cached solver first
                    # returns "Solution may be inaccurate" warnings and then
                    # hard-fails with `Exception: Data formatting error` inside
                    # `_solver.update(P, q, A, b)`.  Rebuilding each time costs
                    # one extra CVXPY compile (sub-millisecond at K<=25) and
                    # avoids both failure modes.
                    cur_K = np.minimum(num_dat,K)
                    online_problem, online_x, online_s, online_tau, online_lmbda, data_train, eps_train, w_train = createproblem_portLP_p2(cur_K, m)
                    data_train.value = k_dict['d'][:num_dat]
                    eps_train.value = radius**2
                    w_train.value = k_dict['w'][:num_dat]

                    if safe_solve(online_problem, name='online_problem_p2', t=t,
            ...
            if t % interval == 0 or ((t-1) % interval == 0)  or (t in t_list) :
                if t <= 8001 or (t in t_list):
                    ...
                    # Rebuild the MRO W2 problem on every solve -- same reason
                    # as the online branch above (quad_over_lin perspective +
                    # Clarabel update path).
                    cur_K_mro = new_k_dict['d'].shape[0]
                    MRO_problem, MRO_x, MRO_s, MRO_tau, MRO_lmbda, MRO_data_train, MRO_eps_train, MRO_w_train = createproblem_portLP_p2(cur_K_mro, m)
                    MRO_data_train.value = new_k_dict['d']
                    MRO_w_train.value = new_k_dict['w']
                    MRO_eps_train.value = radius**2
                    if safe_solve(MRO_problem, name='MRO_problem_p2', t=t,
                                  ignore_dpp=True, solver=cp.CLARABEL, verbose=False, time_limit=1500.0):
            ...
            if t % interval == 0 or ((t-1) % interval == 0) or (t in t_list) :
                if t <= 8001 or (t in t_list):
                    ...
                    new_problem, s_d, lam_d, x_d, tau_d, eps_d, w_d =  worst_case_p2(num_dat,m,running_samples)
                    w_d.value = (1/num_dat)*np.ones(num_dat)
                    eps_d.value = radius**2
                    x_d.value = x_current
                    tau_d.value = tau_current
                    if safe_solve(new_problem, name='worst_case_p2(online)', t=t,
                                  ignore_dpp=True, solver=cp.CLARABEL, verbose=False, time_limit=1500.0):
                ...
                if t % interval == 0 or ((t-1) % interval == 0)  or (t in t_list) :
                    if t <= 8001 or (t in t_list):
                        ...
                        if safe_solve(new_problem, name='worst_case_p2(MRO)', t=t,
                                      ignore_dpp=True, solver=cp.CLARABEL, verbose=False, time_limit=1500.0):
            ...
            if t % interval == 0 or ((t-1) % interval == 0) or (t in t_list) :
                if t <= 8001 or (t in t_list):
                    ...
                    if safe_solve(new_problem, name='worst_case_p2(online,regret)', t=t,
                                  ignore_dpp=True, solver=cp.CLARABEL, verbose=False, time_limit=1500.0):
                        new_worst = new_problem.objective.value
                        worst_time = new_problem.solver_stats.solve_time
                    else:
            ...
            if t % interval == 0 or ((t-1) % interval == 0) or (t in t_list)  :
                if t <= 8001 or (t in t_list):
                    ...
                    if safe_solve(new_problem, name='worst_case_p2(MRO,regret)', t=t,
                                  ignore_dpp=True, solver=cp.CLARABEL, verbose=False, time_limit=1500.0):
                        new_worst_MRO = new_problem.objective.value
                        MRO_worst_time = new_problem.solver_stats.solve_time
                    else:
