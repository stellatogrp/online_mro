...
def port_experiments(r_input,K,T,N_init,synthetic_returns,r_start):
    try:
        ...
        # Initial PCA fit on the first N_init samples; only used by the
        # pre-loop placeholder problem builds below.  The fit is refreshed
        # at the top of every for-loop iteration on the current
        # ``running_samples`` so the low-rank DRO projection tracks the
        # online distribution shift.
        init_eps = eps_init[epsnum]
        num_dat = N_init
        pca_A, pca_b = pca(dat[init_ind:(init_ind+num_dat)], lr_k)
        ...
        online_problem, online_x, online_s, online_tau, online_lmbda, data_train, eps_train, w_train = createproblem_portLP_lr(np.minimum(num_dat,K), m, pca_A, pca_b)
        MRO_problem, MRO_x, MRO_s, MRO_tau, MRO_lmbda, MRO_data_train, MRO_eps_train, MRO_w_train = createproblem_portLP_lr(np.minimum(num_dat,K), m, pca_A, pca_b)
        ...
        for t in range(T):
            ...
            if t % interval == 0 or ((t-1) % interval == 0) or (t in t_list):
                if t <= 8001 or (t in t_list):
                    ...
                    online_problem, online_x, online_s, online_tau, online_lmbda, data_train, eps_train, w_train = createproblem_portLP_lr(cur_K, m, pca_A, pca_b)
            ...
            if t % interval == 0 or ((t-1) % interval == 0)  or (t in t_list) :
                if t <= 8001 or (t in t_list):
                    ...
                    MRO_problem, MRO_x, MRO_s, MRO_tau, MRO_lmbda, MRO_data_train, MRO_eps_train, MRO_w_train = createproblem_portLP_lr(cur_K_mro, m, pca_A, pca_b)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr_k', type=int, default=None,
                        help='Explicit low-rank PCA dim for createproblem_portLP_lr. '
                             'If unset, auto-pick from --lr_var_frac.')
