...
from utils import createproblem_portLP_p2, get_n_processes, safe_solve, save_run_metadata
    ...
    try:
        ...
        for t in range(T):
            ...
            if t % interval == 0 or ((t-1) % interval == 0) or (t in t_list) :
                ...
                # solve DRO problem
                    DRO_problem, DRO_x, DRO_s, DRO_tau, DRO_lmbda, DRO_data, DRO_eps, DRO_w = createproblem_portLP_p2(num_dat,m)
                    DRO_data.value = running_samples
                    DRO_w.value = (1/num_dat)*np.ones(num_dat)
                    DRO_eps.value = radius**2
                    if safe_solve(DRO_problem, name='DRO_problem_p2', t=t,
