...
from utils import compute_cumulative_regret_dro_reg as compute_cumulative_regret
...
def reg_experiments(r_input, T, N_init, synthetic_data, r_start):
    ...
    Regression sibling of ``port_new/port_DRO_orig.py``: the W1-DRO portfolio LP
    is replaced by the perturbed-covariates DRO-BSS mixed-integer SOCP
    (``createproblem_regMIO``) solved with MOSEK, and SAA is the non-robust
    cardinality-constrained least squares (``create_scenario_reg``).
    """
    try:
        ...
        for t in range(T):
            ...
            # radius represents sqrt(delta); RWPI picks sqrt(delta_n) ~ 1/sqrt(n)
            # so the induced ell_2 penalty is statistically principled.
            ...
            if t % interval == 0 or ((t - 1) % interval == 0) or (t in t_list):
                if t <= 2001 or (t in t_list):
                    # solve full-data DRO best subset (MI-SOCP via MOSEK)
                    DRO_problem, DRO_x, DRO_z, DRO_t, DRO_r, DRO_data, DRO_eps, DRO_sqw = createproblem_regMIO(num_dat, m, k)
                    DRO_data.value = running_samples
                    DRO_sqw.value = np.sqrt((1 / num_dat) * np.ones(num_dat))
            ...
            if t % interval_SAA == 0 or ((t - 1) % interval_SAA == 0) or (t in t_list):
                if t <= 2001 or (t in t_list):
                    s_prob, s_x, s_z = create_scenario_reg(running_samples, m, num_dat, k)
