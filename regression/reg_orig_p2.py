...
def reg_experiments(r_input, K, T, N_init, synthetic_data, power, kappa, p, r_start):
    """Online MRO + batch MRO + cluster_SAA sparse-SVM — finite label cost kappa.

    Transport cost: c((x,y),(x',y')) = ||x - x'||_2 + kappa * |y - y'|.
    The DRO problem (Corollary 3.12) is reformulated as an MI-SOCP:
    ...
    kappa -> inf recovers the p=2 hinge (labels fixed, delta*||beta||_2 penalty).
    """
    try:
        ...
        for t in range(T):
            ...
            if t % interval == 0 or ((t - 1) % interval == 0) or (t in t_list):
                if t <= 8001 or (t in t_list):
                    ...
                    online_problem, online_beta, online_z, online_lmbda, online_s, \
                        data_train, eps_train, kappa_param, w_train = \
                        createproblem_hingeMIO_kappa(cur_K, m, k, p=p)
            ...
            if t % interval == 0 or ((t - 1) % interval == 0) or (t in t_list):
                if t <= 8001 or (t in t_list):
                    if t <= fixed_time:
                        ...
                        new_k_dict = {'K': cur_K, 'data': {}, 'a': new_centers, 'd': new_centers, 'w': wk}
                        for kk in range(cur_K):
                            new_k_dict['data'][kk] = running_samples[klabels == kk]

                    cur_K_mro = new_k_dict['d'].shape[0]
                    MRO_problem, MRO_beta, MRO_z, MRO_lmbda, MRO_s, \
                        MRO_data_train, MRO_eps_train, MRO_kappa_param, MRO_w_train = \
                        createproblem_hingeMIO_kappa(cur_K_mro, m, k, p=p)
                    MRO_data_train.value = new_k_dict['d']
                    MRO_w_train.value = new_k_dict['w']
                    MRO_eps_train.value = radius
                    MRO_kappa_param.value = kappa
                    MRO_problem.solve(solver=cp.MOSEK, ignore_dpp=True, verbose=False,
                                      mosek_params=MOSEK_PARAMS)
                    MRO_x_current = MRO_beta.value
            ...
            # ---- worst-case hinge evaluation ----
            if t % interval == 0 or ((t - 1) % interval == 0) or (t in t_list):
                if t <= 8001 or (t in t_list):
                    wc_start = time.time()
                    new_worst = worst_case_hinge_kappa(running_samples, m, x_current, radius, kappa, p=p)
                    worst_time = time.time() - wc_start
                    history['worst_values'].append(new_worst)
                    history['worst_times'].append(worst_time)

                    wc_start = time.time()
                    new_worst_MRO = worst_case_hinge_kappa(running_samples, m, MRO_x_current, radius, kappa, p=p)
            ...
            # ---- worst-case for regret ----
            if t % interval == 0 or ((t - 1) % interval == 0) or (t in t_list):
                if t <= 8001 or (t in t_list):
                    wc_start = time.time()
                    new_worst = worst_case_hinge_kappa(running_samples, m, x_prev, radius, kappa, p=p)
                    worst_time = time.time() - wc_start
            ...
            if t % interval == 0 or ((t - 1) % interval == 0) or (t in t_list):
                if t <= 8001 or (t in t_list):
                    wc_start = time.time()
                    new_worst_MRO = worst_case_hinge_kappa(running_samples, m, MRO_x_prev, radius, kappa, p=p)
                    MRO_worst_time = time.time() - wc_start
...
if __name__ == '__main__':
    ...
    results = Parallel(n_jobs=njobs)(delayed(reg_experiments)(
        r_input, K, T, N_init, synthetic_data, power, kappa, p, r_start) for r_input in range(len(list_inds)))
