...
def reg_experiments(r_input, K, T, N_init, synthetic_data, r_start):
    ...
    try:
        ...
        for t in range(T):
            ...
            q_dict, k_dict, weight_update_time = online_cluster_update(K, new_sample, q_dict, k_dict, num_dat, t, fixed_time, dim, Q)
            if t >= fixed_time:
            ...
            if t % interval == 0 or ((t - 1) % interval == 0) or (t in t_list):
                if t <= 8001 or (t in t_list):
                    ...
                    history['regret_K'].append(w2_dist(k_dict, k_dict_prev, dim) + 2 * radius)
                    history['MRO_regret_K'].append(w2_dist(new_k_dict, new_k_dict_prev, dim) + 2 * radius)
